...
thrust::device_vector<float> d_combined(NUM_SAMPLES);
    ...
    thrust::transform(wave1, wave2 + NUM_SAMPLES, wave2, d_combined.begin(), thrust::plus<float>());
    nvtxRangePop();
    ...
    auto mag = thrust::make_transform_iterator(d_fft.begin(), [] __host__ __device__(cufftComplex c)
