...
struct as_expected_t
{
  ...
  operator()(const ::std::exception*, const ::cuda::std::source_location) const noexcept
  {
...
{
  // Elide `noop` only when the other side is already an stf policy. A bare lambda (or
  // `std::ignore`) is not: `noop & effect` must stay a tagged composite so that a further
  // `& next` still finds this operator -- that is why `noop` heads chains of plain effects.
  if constexpr (::cuda::std::is_same_v<::cuda::std::remove_cvref_t<_L>, noop_t> && detail::__is_stf_policy_v<_R>)
  {
    return detail::__normalize(::cuda::std::forward<_R>(__r));
  }
  else if constexpr (::cuda::std::is_same_v<::cuda::std::remove_cvref_t<_R>, noop_t> && detail::__is_stf_policy_v<_L>)
  {
...
auto operator|(_L&& __l, _R&& __r)
{
  // Same elision rule as `&`: only drop `rethrow` when the survivor is already tagged.
  if constexpr (::cuda::std::is_same_v<::cuda::std::remove_cvref_t<_L>, rethrow_t> && detail::__is_stf_policy_v<_R>)
  {
    return detail::__normalize(::cuda::std::forward<_R>(__r));
  }
  else if constexpr (::cuda::std::is_same_v<::cuda::std::remove_cvref_t<_R>, rethrow_t> && detail::__is_stf_policy_v<_L>)
  {
    return detail::__normalize(::cuda::std::forward<_L>(__l));
  }
  else
